    ...
    *)
    ...
    and map_structure_item item =
      ...
      let str_desc =
        match item.str_desc with
          ...
          | Tstr_primitive (id, v) -> Tstr_primitive (id, map_value_description v)
          | Tstr_type list -> Tstr_type (List.map (fun (id, decl) -> (id, map_type_declaration decl) ) list)
          | Tstr_exception (id, decl) -> Tstr_exception (id, map_exception_declaration decl)
          | Tstr_exn_rebind (id, path) -> Tstr_exn_rebind (id, path)
          | Tstr_module (id, mexpr) -> Tstr_module (id, map_module_expr mexpr)
          | Tstr_recmodule list ->
	    let list =
              List.map (fun (id, mtype, mexpr) ->
                (id, map_module_type mtype, map_module_expr mexpr)
	      ) list
	    in
	    Tstr_recmodule list
          | Tstr_modtype (id, mtype) -> Tstr_modtype (id, map_module_type mtype)
          | Tstr_open path -> Tstr_open path
          | Tstr_class list ->
	    let list =
    ...
    and map_type_declaration decl =
	 ...
	 loc)
      ...
      let typ_kind = match decl.typ_kind with
        ...
        | Ttype_variant list ->
          let list = List.map (fun (s, cts, loc) ->
            (s, List.map map_core_type cts, loc)
          ) list in
	  Ttype_variant list
        | Ttype_record list ->
	  let list =
            List.map (fun (s, mut, ct, loc) ->
              (s, mut, map_core_type ct, loc)
            ) list in
	  Ttype_record list
      in
      let typ_manifest =
    ...
    and map_pattern pat =
	  ...
	  | Tpat_tuple list -> Tpat_tuple (List.map map_pattern list)
          | Tpat_construct (path, cstr_decl, args) -> Tpat_construct (path, cstr_decl, List.map map_pattern args)
	    ...
	    Tpat_variant (label, pato, rowo)
          | Tpat_record (list, closed) ->
            Tpat_record (List.map (fun (path, lab_desc, pat) -> (path, lab_desc, map_pattern pat) ) list, closed)
    ...
    and map_expression exp =
      ...
      let exp_desc =
	    ...
	    Texp_let (rec_flag,
          ...
          | Texp_construct (path, cstr_desc, args) ->
            Texp_construct (path, cstr_desc,  List.map map_expression args )
	    ...
	    let list =
              List.map (fun (path, lab_desc, exp) ->
                (path, lab_desc, map_expression exp)
              ) list in
            let expo = match expo with
                None -> expo
              | Some exp -> Some (map_expression exp)
	    ...
	    Texp_record (list, expo)
          | Texp_field (exp, path, label) ->
            Texp_field (map_expression exp, path, label)
	  ...
	  | Texp_new (path, cl_decl) -> exp.exp_desc
	  | Texp_instvar (_, path) -> exp.exp_desc
	  | Texp_setinstvar (path, path2, exp) ->
	    Texp_setinstvar (path, path2, map_expression exp)
	  | Texp_override (path, list) ->
	    Texp_override (
	      path,
	      List.map (fun (path, exp) ->
		(path, map_expression exp)
	      ) list
	    )
	  ...
	  | Texp_open (path, exp) -> Texp_open (path, map_expression exp)
	  | Texp_newtype (s, exp) ->
    ...
    and map_signature_item item =
      ...
      let sig_desc =
        match item.sig_desc with
            Tsig_value (id, v) -> Tsig_value (id, map_value_description v)
          | Tsig_type list -> Tsig_type (
            List.map (fun (id, decl) ->
              (id, map_type_declaration decl)
            ) list
	  )
          | Tsig_exception (id, decl) ->
            Tsig_exception (id, map_exception_declaration decl)
          | Tsig_module (id, mtype) ->
            Tsig_module (id, map_module_type mtype)
          | Tsig_recmodule list ->
            Tsig_recmodule (List.map (fun (id, mtype) -> (id, map_module_type mtype) ) list)
          | Tsig_modtype (id, mdecl) ->
            Tsig_modtype (id, map_modtype_declaration mdecl)
          | Tsig_open path -> item.sig_desc
          | Tsig_include mty -> Tsig_include (map_module_type mty)
    ...
    and map_with_constraint cstr =
      ...
      let cstr =
        match cstr with
          ...
          | Twith_module path -> cstr
          | Twith_modsubst path -> cstr
      in
    ...
    and map_module_expr mexpr =
      ...
      let mod_desc =
        match mexpr.mod_desc with
            Tmod_ident p -> mexpr.mod_desc
          | Tmod_structure st -> Tmod_structure (map_structure st)
          | Tmod_functor (id, mtype, mexpr) ->
            Tmod_functor (id, map_module_type mtype, map_module_expr mexpr)
        ...
        match cexpr.cl_desc with
          ...
          | Tcl_fun (label, pat, priv, cl, partial) ->
            Tcl_fun (label, map_pattern pat,
		     List.map (fun (id, exp) -> (id, map_expression exp)) priv,
          ...
          | Tcl_let (rec_flat, bindings, ivars, cl) ->
            Tcl_let (rec_flat, map_bindings rec_flat bindings,
		     List.map (fun (id, exp) -> (id, map_expression exp)) ivars,
		     map_class_expr cl)
          ...
          | Tcl_ident (id, tyl) ->
            Tcl_ident (id, List.map map_core_type tyl)
      in
      ...
      let cltyp_desc =
        match ct.cltyp_desc with
          ...
          | Tcty_constr (path, list) ->
            Tcty_constr (path, List.map map_core_type list)
    ...
    and map_core_type ct =
      ...
      let ctyp_desc =
        match ct.ctyp_desc with
          ...
          | Ttyp_constr (path, list) -> Ttyp_constr (path, List.map map_core_type list)
          | Ttyp_object list -> Ttyp_object (List.map map_core_field_type list)
          | Ttyp_class (path, list, labels) -> Ttyp_class (path, List.map map_core_type list, labels)
    ...
    and map_class_field cf =
	  ...
	  | Tcf_val (lab, mut, ident, Tcfk_virtual cty, override) ->
            Tcf_val (lab, mut, ident, Tcfk_virtual (map_core_type cty), override)
	  | Tcf_val (lab, mut, ident, Tcfk_concrete exp, override) ->
            Tcf_val (lab, mut, ident, Tcfk_concrete (map_expression exp), override)
	  | Tcf_meth (lab, priv, Tcfk_virtual cty, override) ->
            Tcf_meth (lab, priv, Tcfk_virtual (map_core_type cty), override)
	  | Tcf_meth (lab, priv, Tcfk_concrete exp, override) ->
            Tcf_meth (lab, priv, Tcfk_concrete (map_expression exp), override)
	  | Tcf_let (rec_flag, bindings, exps) ->
            Tcf_let (rec_flag, map_bindings rec_flag bindings,
	    List.map (fun (id, exp) -> (id, map_expression exp)) exps)
